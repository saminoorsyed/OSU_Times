...
INSERT INTO Users2 (full_name, username, email)
    VALUES
...
INSERT INTO Authors2 (full_name, username, email, admin_action, admin_id)
    VALUES
        ...
        ("Tuvok Vulcan", "Commander", "tvuclan@voyager.com", "1) on 11.3.4060 Supreme Commander expressed concern on post content 2)Warned and put on probation on 11.4.4060",
            (SELECT admin_id FROM Administrators2 WHERE full_name = "Mister Neelix" )),
        ("Seven ofNine", "Borg", "sevenofnine@borg.com", "1)Promote her work to front page (Supreme commander expressed approval)",
            (SELECT admin_id FROM Administrators2 WHERE full_name = "Belana Torres" )),
...
INSERT INTO Users_Authors2 (user_id, author_id)
    VALUES
        ((Select user_id from Users2 WHERE full_name = "Kathryn Janeway"),
            (SELECT author_id FROM Authors2 WHERE full_name = "Seven ofNine")),
        ((Select user_id from Users2 WHERE full_name = "Kathryn Janeway"),
            (SELECT author_id FROM Authors2 WHERE full_name = "Tuvok Vulcan")),
        ((SELECT user_id FROM Users2 WHERE full_name = "Amal Chakotay"),
            (SELECT author_id FROM Authors2 WHERE full_name = "Tuvok Vulcan"));
...
INSERT INTO Posts2 (title, post_text, author_id, genre_id)
    VALUES
        ("Ethics of the Collective", 
            ...
            (SELECT author_id FROM Authors2 WHERE username = "Borg"),
            (SELECT genre_id FROM Genres2 WHERE genre_name = "Social Science")),
        ("An Anthropology of space farring cultures",
            ...
            (SELECT author_id FROM Authors2 WHERE username = "TheDoctor"),
            (SELECT genre_id FROM Genres2 WHERE genre_name="Opinions")),
...
INSERT INTO Comments2 (comment_text, post_id, user_id)
    VALUES
        ("Great article Doctor!",
            (SELECT post_id FROM Posts2 WHERE title = "An Anthropology of space farring cultures"),
            (SELECT user_id FROM Users2 WHERE username = "Commander")),
        ("Novel insight into the collective!",
            (SELECT post_id FROM Posts2 WHERE title = "Ethics of the Collective"),
            (SELECT user_id FROM Users2 WHERE username = "Captain")),
        ("I hate the Borg!!!",
            (SELECT post_id FROM Posts2 WHERE title = "Ethics of the Collective"),
            (SELECT user_id FROM Users2 WHERE username = "Captain"));
...
INSERT INTO Reactions2 (user_id, post_id, reaction_icon_id)
    VALUES
        ((SELECT user_id FROM Users2 WHERE full_name = "Amal Chakotay"),
            (SELECT post_id FROM Posts2 WHERE title = "An Anthropology of space farring cultures"),
            (SELECT reaction_icon_id FROM Reaction_Icons2 WHERE reaction_type = "heart")),
        ((SELECT user_id FROM Users2 WHERE full_name = "Amal Chakotay"),
            (SELECT post_id FROM Posts2 WHERE title = "Ethics of the Collective"),
            (SELECT reaction_icon_id FROM Reaction_Icons2 WHERE reaction_type = "like")),
        ((SELECT user_id FROM Users2 WHERE full_name = "Kathryn Janeway"),
            (SELECT post_id FROM Posts2 WHERE title = "Ethics of the Collective"),
            (SELECT reaction_icon_id FROM Reaction_Icons2 WHERE reaction_type = "like"));
